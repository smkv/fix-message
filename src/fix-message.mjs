...
class FixMessageHTMLElement extends HTMLElement {
    ...
    async renderTable() {
        ...
        const copyMessage = document.createElement('a');
        const copyIcon = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-clipboard" viewBox="0 0 16 16">\n' +
        ...
        const copiedIcon = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-clipboard-check" viewBox="0 0 16 16">\n' +
            ...
            '  <path d="M9.5 1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5zm-3-1A1.5 1.5 0 0 0 5 1.5v1A1.5 1.5 0 0 0 6.5 4h3A1.5 1.5 0 0 0 11 2.5v-1A1.5 1.5 0 0 0 9.5 0z"/>\n' +
            '</svg> Copy';
        copyMessage.innerHTML = copyIcon;
        copyMessage.href = `#`;
        copyMessage.style.textDecoration = 'none';
        copyMessage.style.fontSize = 'smaller';
        copyMessage.title = 'Copy message to clipboard';
        copyMessage.addEventListener('click', e => {
            e.preventDefault();
            navigator.clipboard.writeText(this.messageWithDelimiter);
            copyMessage.innerHTML = copiedIcon;
            this.dispatchEvent(new CustomEvent('copied'));
        });
        messageDiv.append(copyMessage);
